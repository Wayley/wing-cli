...
Creator.prototype.create = async function () {
  ...
  if (framework === 'vue.iview') {
    console.log('\nVue with iView will come sonn, just waitting for it...');
    return;
  }
  //
  ...
  // 写入template
  const template = `node_modules/wing-cli-service/generator/template/${framework}`;
  ...
  await writeFiles(targetDir, {
    'package.json': JSON.stringify(
      Object.assign({ name: appName, version }, pkg),
      null,
      2
    ),
  });
  // 添加.gitignore
  await fs.rename(`${targetDir}/_gitignore`, `${targetDir}/.gitignore`);
